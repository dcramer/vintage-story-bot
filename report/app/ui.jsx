...
export function GoalLine({ g }) {
  ...
  return <div class="goal">
    <div class="headline"><span class="kind">{g.kind.replace(/_/g, ' ')}</span><b>{goalTitle(g)}</b>
    ...
    <div class="phase">{p.phase ? <><b>{p.phase.replace(/_/g, ' ')}</b>{phaseDetail(p) ? ' — ' + phaseDetail(p) : ''}</> : g.reason ? <span class="muted">{g.reason}</span> : null}</div>
  </div>;
}
