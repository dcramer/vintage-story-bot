...
public sealed class InventoryAdapter(ICoreClientAPI api)
{
    ...
    private object SlotInfo(ItemSlot? slot, int index)
    {
        ...
        return new { slot = index, code = stack?.Collectible.Code.ToString(), quantity = slot?.StackSize ?? 0,
            itemClass = stack?.Class.ToString(), bag = slot is ItemSlotBackpack ? true : (bool?)null,
    ...
    public object Drop(JsonElement request)
    {
        ...
        mutation++;
        if (source.Inventory == Manager.GetOwnInventory("mouse")) Manager.DropMouseSlotItems(full);
        else Manager.DropItem(source, full);
        return new { ok = true, status = "submitted", code, dropped, state = State() };
    }
