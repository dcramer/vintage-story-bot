...
- Never commit world-specific locations or connection details as behavior: no server addresses, credentials, spawn/home/resource coordinates or player-specific waypoints in source or committed config. Supply connection details through runtime config/environment, pass destinations as goal inputs, and discover or persist places per world in ignored `.runtime` memory. Tests use clearly synthetic coordinates only.
