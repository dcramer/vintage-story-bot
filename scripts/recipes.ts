...
//   node scripts/recipes.mjs [--assets <dir>]   (default .runtime/linux-client/assets/survival)
import { readdirSync, readFileSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';

const args = process.argv.slice(2);
const root = args[args.indexOf('--assets') + 1] && args.includes('--assets') ? args[args.indexOf('--assets') + 1]
...
export function parseLenient(text) {
  ...
  const string = quote => {
    ...
    i++;
    while (text[i] !== quote) {
      if (text[i] === '\\') { const c = text[++i]; out += { n: '\n', t: '\t', r: '\r' }[c] ?? c; }
      else out += text[i];
      i++;
    }
    i++;
  ...
  const value = () => {
    ...
    if (c === '{') {
      ...
      for (;;) {
        ...
        const key = text[i] === '"' || text[i] === "'" ? string(text[i]) : (() => { let k = ''; while (/[\w$.\-*]/.test(text[i])) k += text[i++]; return k; })();
        skip(); i++; // colon
        obj[key] = value();
        skip();
        if (text[i] === ',') i++;
...
const ingredient = (ref, key?) => ref && {
  ...(key !== undefined ? { key } : {}), type: ref.type ?? 'item', code: code(ref), ...(ref.tags ? { tags: ref.tags } : {}), quantity: ref.quantity ?? 1,
  ...(ref.name ? { variable: ref.name } : {}), ...(ref.allowedVariants ? { allowedVariants: ref.allowedVariants } : {}),
  ...(ref.skipVariants ? { skipVariants: ref.skipVariants } : {}), ...(ref.isTool ? { tool: true, durabilityCost: ref.toolDurabilityCost ?? 1 } : {}),
  ...(ref.consumeQuantity != null ? { consumeQuantity: ref.consumeQuantity } : {}),
  ...(ref.attributes ? { attributes: ref.attributes } : {}), ...(ref.isWildCard ? { wildcard: true } : {}),
  ...(ref.litres != null ? { litres: ref.litres } : {}),
};
const output = ref => ref && { type: ref.type ?? 'item', code: code(ref), quantity: ref.quantity ?? 1,
  ...(ref.attributes ? { attributes: ref.attributes } : {}), ...(ref.litres != null ? { litres: ref.litres } : {}) };
const voxels = pattern => list(pattern).flat().join('').split('').filter(c => c === '#').length;

function grid(recipe) {
  const pattern = String(recipe.ingredientPattern ?? '').split(/[,\t]/).map(row => row.trim()).filter(Boolean);
  const used = new Set(pattern.join('').replace(/_/g, ''));
  return {
    kind: 'grid', name: recipe.name ?? null, output: output(recipe.output), width: recipe.width ?? pattern[0]?.length ?? 1, height: recipe.height ?? pattern.length,
    shapeless: recipe.shapeless ?? false, pattern,
    ingredients: Object.entries(recipe.ingredients ?? {}).filter(([key]) => used.has(key)).map(([key, ref]) => ingredient(ref, key)),
...
const forming = kind => recipe => ({ kind, name: recipe.name ?? null, output: output(recipe.output), ingredient: ingredient(recipe.ingredient),
  layers: list(recipe.pattern).length, voxels: voxels(recipe.pattern), pattern: recipe.pattern });
const cooking = recipe => ({ kind: 'cooking', name: recipe.code ?? null, output: output(recipe.output), perishable: recipe.perishableHours ?? null,
  ingredients: (recipe.ingredients ?? []).map(slot => ({ code: slot.code, minQuantity: slot.minQuantity ?? 0, maxQuantity: slot.maxQuantity ?? 1,
    validStacks: (slot.validStacks ?? []).map(stack => ({ type: stack.type ?? 'item', code: code(stack), ...(stack.shapeless != null ? {} : {}),
      ...(stack.cookedStack ? { cooked: output(stack.cookedStack) } : {}) })) })) });
const barrel = recipe => ({ kind: 'barrel', name: recipe.code ?? null, output: output(recipe.output), sealHours: recipe.sealHours ?? 0,
  ingredients: (recipe.ingredients ?? []).map(ref => ingredient(ref)) });
const smithing = recipe => ({ kind: 'smithing', name: recipe.name ?? null, output: output(recipe.output), ingredient: ingredient(recipe.ingredient),
  layers: list(recipe.pattern).length, voxels: voxels(recipe.pattern) });
const alloy = recipe => ({ kind: 'alloy', output: output(recipe.output), ingredients: (recipe.ingredients ?? []).map(ref => ({ code: code(ref),
  minRatio: ref.minRatio, maxRatio: ref.maxRatio })) });
...
for (const [kind, parse] of Object.entries(parsers)) {
  for (const file of files(join(recipesDir, kind))) {
    let parsed;
    try { parsed = parseLenient(readFileSync(file, 'utf8')); }
    catch (error) { console.error(`skip ${file}: ${error.message}`); continue; }
...
const wild = ['stick', 'stone-*', 'flint', 'obsidian', 'clay-*', 'cattailtops', 'cattailroot', 'papyrustops', 'papyrusroot', 'drygrass', 'log-*',
  'bone', 'hide-*', 'fat', 'resin', 'sand-*', 'gravel-*', 'soil-*', 'rock-*', 'treeseed-*', 'seeds-*', 'fruit-*', 'vegetable-*', 'mushroom-*',
  'grain-*', 'flaxfibers', 'feather', 'egg-*', 'honeycomb', 'peat', 'waterportion', 'saltwaterportion', 'bamboo-*', 'seashell-*', 'leaves-*',
  'sapling-*', 'fern', 'tallgrass-*', 'nugget-*', 'charcoal', 'reeds-*', 'cactus-*', 'poultry-*', 'redmeat-*', 'bushmeat-*', 'fish-*', 'insect-*',
  'snowlayer', 'ice', 'looseboulders-*', 'looseflints-*', 'loosestones-*', 'flower-*', 'thatch', 'hay-*'];
const prefix = pattern => String(pattern ?? '').replace(/^game:/, '').replace(/[*{].*$/, '');
const isWild = pattern => /[*{]/.test(String(pattern ?? ''));
const satisfied = (input, known) => {
  if (input.tags) return list(input.tags).flat(Infinity).map(String).some(tag => tag.startsWith('tool-') && [...known].some(k => k.startsWith(`${tag.slice(5)}-`)));
  if (!input.code) return false;
  const want = prefix(input.code);
  if (want === '') return true;
  return [...known].some(k => isWild(input.code) ? k.startsWith(want) || (isWild(k) && want.startsWith(prefix(k))) : k === want || (isWild(k) && want.startsWith(prefix(k))));
};
