...
if (params[0] === '--json' && params.length === 2) {
  try { args = JSON.parse(params[1]); expectedParams = 2; }
  catch { console.error('Invalid JSON arguments.'); process.exit(1); }
}
const parsed = tool?.schema.safeParse(args);
if (!parsed?.success || params.length !== expectedParams) {
  console.error('Usage: control.mjs <action> --json <object>, or observe|scan|stop|move <ms>|look <yaw> <pitch>|select <slot>|interact <ms>|attack <ms>');
  process.exitCode = 1;
} else {
  try {
