...
- [ ] **P1 · finding things by where they occur** — a search should start from what the handbook says about where a thing is found (sticks under trees and in branchy leaves, loose flint and stones on bare ground and by rock, cattails and reeds at water, berries in the open) and head for such places from far-view memory, instead of looking around and walking the nearest heading; `gather`, `harvest` and `forage` share it (`support`). Asked for 2026-09-11.
