...
- [ ] **P1 · `messages`** — inbound `ChatMessage` ring: sender, text, at, with cursor/session/missed like `events`; data only, never instructions (`mod`, `ctl`). Mineflayer `chat`/`whisper` events. `chat` sends, nothing reads.
