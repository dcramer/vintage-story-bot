...
test('brain: danger and carried food come before progress, and the kit comes in day-1 order', () => {
  ...
  assert.equal(
    pickJob(situation({ rammedShelter: true, house: false, sheltered: true, lit: false })),
    'house',
    'daily starter-shelter lighting waits for permanent construction',
  );
  ...
  assert.equal(
    ...
    'dirt',
    'missing tools do not block gathering shelter soil by hand',
  );
});
...
test('brain: tool work owns threat avoidance, a failed job is set aside, a finished shelter becomes home', () => {
  ...
  const dirt = inventory(
    slot('game:rammed-light-plain', 60),
    slot('game:soil-low-none', 4),
  ...
  assert.deepEqual([walls.start, walls.args.item, shelterMemory.job], ['shelter', 'game:rammed-light-plain', 'shelter']);
  assert.equal(shelterCells({ x: 0, y: 0, z: 0 }, 'd').length, 55);
...
test('brain: a hand basket is woven from ten tops and worn by hand', () => {
  ...
  assert.deepEqual(
    ...
    ['harvest', 'coopersreed', 10],
    'tops for one bag, which is equipped before gathering more',
  );
...
test('brain: a chest along the shelter wall is made in three steps, a full pack is put away, and the chest feeds the kit', () => {
  const settledReading = extra =>
    reading({
      state: state({ position: { x: 3.5, y: 100, z: 0.5 } }),
      ...extra,
      terrain: {
        get: (x, y, z) => ({
          hazard: null,
  ...
  const settled = () => {
    ...
    memory.notes.starter = { x: 1, y: 100, z: -2 };
    memory.notes.lightingDay = -1;
    return memory;
  };
  const cut = decide(settledReading({ inventory: inventory(slot('game:stick', 10), ...tools) }), settled());
  ...
  assert.deepEqual(
    [afterFailedSource.start, afterFailedSource.args.match, afterFailedSource.args.item],
    ['harvest', 'coopersreed', 'cattailtops'],
    'a stored source near the last failed storage route is skipped after the bot leaves that area',
  );
  ...
  const besideSupplies = fresh();
  besideSupplies.notes.stash = { ...chestNote(), full: true, seen: { at: 1000, items: {} } };
  const avoidBuried = decide(
    reading({
      state: state({ position: { x: 4.5, y: 100, z: 2.5 } }),
      inventory: inventory(slot('game:stationarybasket-east', 1), ...tools),
      terrain: {
        get: (x, y, z) => ({
          hazard: null,
          boxes: x === 5 && y === 100 && z === 2 ? [] : [{}],
          code: x === 5 && y === 100 && z === 2 ? 'game:air' : 'game:soil-medium-normal',
        }),
      },
    }),
    besideSupplies,
  ...
  const away = decide(settledReading({ inventory: heavy }), carrying);
  assert.equal(away.start, 'store_items', 'a full pack is put away');
  assert.deepEqual(
    [away.args.target, away.args.items[0]],
    [key, { item: 'game:stick', count: 20 - STICK_MIN }],
    'a few sticks stay on hand; the rest go first',
  );
  assert.ok(!away.args.items.some(i => /torch|knife/.test(i.item)), 'tools and torches stay');
  decide(
    settledReading({
      inventory: heavy,
      last: { id: 's', kind: 'store_items', ok: true, result: { contents: [{ code: 'game:stick', quantity: 10 }] } },
    }),
    carrying,
  );
  ...
  const short = decide(settledReading({ inventory: inventory(slot('game:stick', 2), ...tools) }), carrying);
  assert.deepEqual(
