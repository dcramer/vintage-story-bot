...
test('threat avoidance is explicit, proximity-bounded and points away', () => {
  ...
  assert.equal(nearestThreat({ position: player, nearbyEntities: [{ ...bowtorn, point: { x: 46.5, y: 2, z: 10.5 } }] }).code, 'game:bowtorn-surface');
...
test('a grounded route nudge is short, sneaking, deterministic and measured', async () => {
  const state = x => ({
    ...
    mounted: false,
    player: { uid: 'test' },
    position: { x, y: 1, z: 0.5, dimension: 0 },
    body: { halfWidth: 0.3, height: 1.85 },
    motion: { onGround: true, swimming: false, feetInLiquid: false },
  ...
  const env = {
    ...
    send: async request => {
      ...
      if (request.action === 'stop') latest = state(0.45);
      return { ok: true };
    },
  };
  const field = new Fieldwork(env, { wait: async () => {}, now: () => 0 });
  field.initial = field.latest = latest;
  assert.equal(await field.nudge({ x: 5.5, y: 1, z: 0.5 }), 0.35);
  assert.deepEqual(aimed, { yawDegrees: 90, pitchDegrees: 15 });
  assert.deepEqual(movement, { action: 'move', durationMs: 400, direction: 'forward', jump: false, sprint: false, sneak: true });
  assert.equal(field.moved, 0.35);
});
