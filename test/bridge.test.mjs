...
import { requestBridge, bridgePort } from '../src/runtime/bridge.mjs';
import { tools as actions } from '../src/runtime/registry.mjs';
...
test('MCP stdio discovery, every tool mapping, validation, and error propagation', async t => {
  const calls = [];
  let refuse = false;
  let capabilities = ['target_guard', 'directional_move'];
  const port = await fakeBridge(t, (socket, request) => {
    calls.push(request);
    socket.end(JSON.stringify(refuse ? { ok: false, error: 'World paused' } : { ok: true, capabilities, request }) + '\n');
  });
  ...
  await client.connect(new StdioClientTransport({
    ...
    cwd: '/tmp',
    env: { ...process.env, VINTAGE_STORY_CONTROLLER_PORT: String(port) },
    stderr: 'pipe',
  }));
  const { tools } = await client.listTools();
  assert.deepEqual(tools.map(tool => tool.name).sort(), actions.map(tool => tool.name).sort());
  assert.equal(calls.length, 0, 'startup and discovery must not contact the game');
  assert.equal(tools.find(tool => tool.name === 'observe').annotations.readOnlyHint, true);
  assert.equal(tools.find(tool => tool.name === 'attack_block').annotations.destructiveHint, true);
  for (const [name, args, action] of [
    ['observe', {}, 'observe'], ['scan', { radius: 4, limit: 8, kind: 'blocks', match: 'stick' }, 'scan'], ['look', { yawDegrees: 90, pitchDegrees: 20 }, 'look'],
    ['move', { durationMs: 250, direction: 'left', jump: false }, 'move'], ['select_hotbar', { slot: 2 }, 'select'],
    ['interact', { durationMs: 100, expectedTarget: 'block:0:1:2:3:game:loosestick-free' }, 'interact'], ['attack_block', { durationMs: 100 }, 'attack'],
    ['stop', {}, 'stop'],
    ['move_to', { x: 12.5, y: 100, z: 8.5, dimension: 0, timeoutMs: 5000 }, 'move_to'],
    ['gather_sticks', { count: 1 }, 'gather_sticks'],
    ['events', { after: 12, session: 'test' }, 'events'], ['respawn', { deathId: 'test:1' }, 'respawn'],
    ['inventory', {}, 'inventory'], ['recipes', { match: 'firestarter' }, 'recipes'],
    ['inventory_move', { from: { inventory: 'hotbar', slot: 0 }, to: { inventory: 'craftinggrid', slot: 0 }, quantity: 1, expectedState: 'a'.repeat(64) }, 'inventory_move'],
    ['craft', { to: { inventory: 'hotbar', slot: 0 }, expectedState: 'a'.repeat(64), expectedOutput: 'game:firestarter' }, 'craft'],
  ]) {
    const result = await client.callTool({ name, arguments: args });
    assert.equal(result.isError, false);
    assert.deepEqual(JSON.parse(result.content[0].text).request, { action, ...args });
  }
  const count = calls.length;
  const invalid = await client.callTool({ name: 'move', arguments: { durationMs: 9999 } });
  assert.equal(invalid.isError, true);
  assert.equal(calls.length, count);
  refuse = true;
  const error = await client.callTool({ name: 'observe', arguments: {} });
  assert.equal(error.isError, true);
  assert.match(error.content[0].text, /World paused/);
});
