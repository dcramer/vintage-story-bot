...
test('goal scripts compile a TypeScript-compatible chain through existing goal schemas', () => {
  ...
  assert.deepEqual(
    plan.map(step => [step.goal.name, step.args]),
    [
      ['forage', { count: 32 }],
      ['travel', { x: -181, z: 524 }],
    ],
...
test('controller retains high-level intent and source in the outer goal record', async () => {
  ...
  const request = { action: 'goal_script', intent: 'Bring provisions home', goalScript: 'await goals.travel({ waypoint: "missing" });' };
