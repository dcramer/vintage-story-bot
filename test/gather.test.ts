...
function fixture({ gain = true, interruptAfter = Infinity } = {}) {
  ...
  const env = {
    ...
    sync: async () => state(), aim: async () => {}, report: p => reports.push(p),
    navigate: async () => { walks++; return { state: walks === 1 ? 'blocked' : 'arrived', reason: walks === 1 ? 'no_observed_route' : 'destination_reached' }; },
    send: async request => {
      calls.push(request);
      ...
      if (request.action === 'interact') { picked = true; if (gain) inventory++; return { ok: true }; }
...
test('being hurt is reported and the goal carries on; a life alert ends it', async () => {
  ...
  g.env.send = async request => { const r: any = await first(request); if (request.action === 'observe') r.life.alerts = ['low_health']; return r; };
