...
test('fleet reporter merges surface sweeps into one bounded map delta', async () => {
  ...
  const reporter = new Reporter({ url: 'https://fleet.test', token: 'token', id: 'Atlas', intervalMs: 60000,
    mapColumns: 3, fetch: async (_url, request) => { report = JSON.parse(request.body as string); return new Response(null, { status: 204 }); } });
...
test('fleet reporter keeps goal intent, script and subgoal', async () => {
  ...
  const reporter = new Reporter({ url: 'https://fleet.test', token: 'token', id: 'Kiln', intervalMs: 60000,
    fetch: async (_url, request) => { report = JSON.parse(request.body as string); return new Response(null, { status: 204 }); } });
