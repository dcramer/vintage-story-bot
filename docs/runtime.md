...
- After three consecutive stationary route failures, travel or food recovery (including food search before a lead is visible) may hand-break one body-level, native-reachable leaf block, then remap. It selects the nearest visible surface first so the native target guard cannot be occluded by another leaf, using intended heading as a tie-breaker. This escape hatch requires cached server build/break permission and an empty hotbar slot; it never selects logs, ground, unknown blocks or out-of-reach foliage.
