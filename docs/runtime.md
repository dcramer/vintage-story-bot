...
- Terrain memory is session-local, bounded and expiring; changed/unloaded cells invalidate routes. Identical native observations refresh locally without flooding the RPC stream and periodically republish timestamps. Cached geometry is observation, not proof that terrain is unchanged. Unknown cells are blocked. No persistent POI store.
...
- `forage`: ripe berries, allowlisted safe mushrooms or mature wild crops with safe raw drops → empty-hand harvest → fresh-food equip/eat → satiety/reserve verification. Berries use normal right-click; mushrooms/crops use normal block breaking. `eat`: one verified consumption from inventory. Unknown, poisonous, psychedelic or spoiling food is refused; no raw meat/cassava. Food in backpack needs a free ordinary hotbar slot.
