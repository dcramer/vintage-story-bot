...
- Fleet report: multiple bots on the same codebase report to one Cloudflare Worker + SQLite Durable Object (free plan at ≤5 bots on the 10s interval: ~9k requests/bot/day against the 100k/day limit). One-time setup from repo: `pnpm dlx wrangler@4 login`, `pnpm dlx wrangler@4 secret put REPORT_TOKEN -c report/wrangler.jsonc` (optional `VIEW_TOKEN` gates the page), then `pnpm report:deploy` prints the `workers.dev` URL. Per bot: set `VINTAGE_STORY_REPORT_URL`, `VINTAGE_STORY_REPORT_TOKEN`, and a distinct `VINTAGE_STORY_BOT_ID` (default hostname) in `.env`; `pnpm controller` loads `.env`. Local check: copy `report/.dev.vars.example` to `report/.dev.vars`, `pnpm report:dev`, point the URL at `http://127.0.0.1:8787`. [Contract](architecture.md#telemetry).
