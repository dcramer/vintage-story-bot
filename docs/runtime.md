...
- Navigation observes nearby living entities through RPC on every sensed control step. Explicit vanilla hostile code families within 16 blocks trigger a mapped 24-block route away, with emergency sprint across short level segments allowed above 10% food; the original destination resumes only after the hostile leaves the proximity neighborhood. Unknown/modded entities and neutral wildlife do not trigger avoidance. Any actual damage still cancels immediately.
