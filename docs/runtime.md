...
- Navigation observes nearby living entities through RPC on every sensed control step. Explicit vanilla hostile code families within 32 blocks trigger a mapped 32-block route away, with emergency sprint across short level segments allowed above 10% food; the original destination resumes only after the hostile leaves the proximity neighborhood. Food recovery also checks this proximity before scans and harvests, evades first, then resumes. Unknown/modded entities and neutral wildlife do not trigger avoidance. Any actual damage still cancels immediately.
