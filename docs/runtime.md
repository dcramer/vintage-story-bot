...
- Terrain memory is session-local, bounded and expiring; changed/unloaded cells invalidate routes. Identical native observations refresh locally without flooding the RPC stream and periodically republish timestamps. Cached geometry is observation, not proof that terrain is unchanged. Unknown cells are blocked. Planned standing/traversal centers also require a 0.55-block horizontal braking margin from observed liquid/fire hazards. No persistent POI store.
