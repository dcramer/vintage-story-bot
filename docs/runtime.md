...
- Navigation observes nearby living entities through RPC on every sensed control step. Explicit vanilla hostile code families within 32 horizontal and eight vertical blocks trigger a mapped 32-block route that maximizes clearance from the full visible hostile perimeter; ranged bowtorns use a 24-block vertical window. Accepted route cells cannot materially approach any detected threat. Emergency sprint across short level segments is allowed above 10% food, and the original destination resumes only after the hostile perimeter leaves the proximity neighborhood. Food recovery also checks this proximity before scans and harvests, evades first, then resumes. Unknown/modded entities and neutral wildlife do not trigger avoidance. Any actual damage still cancels immediately.
