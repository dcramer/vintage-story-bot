...
- `control_begin {owner,session,epoch,allowStarvingRecovery?}`: owner 32-hex UUID; observed life session/control epoch; exclusive 500ms initial lease. Every accepted frame renews the ownership heartbeat for 500ms independently of its 1–500ms input duration, so a short steering correction does not shrink the next network deadline or extend its own held keys. The optional recovery scope permits low health only while low food is also active; the lease retains that scope until release.
