...
- `scan`: nearby awareness ≤8 blocks, distant 120×90° sight ≤64. `match` filters one code substring; `matches` filters 1–4 alternatives in one volume pass. Both require loaded sightlines. Pages target ≤12ms/128 rays/32768 cells; native calls can exceed budget. Cursor fixes origin/filter/view, expires after 30s, movement >2 or distant-view rotation >15°. Four cursors, ≤4096 results each. `more` continues enumeration; `incomplete` also flags unloaded/truncated data. Not a world snapshot; revalidate before interaction. Distant object sightings do not imply a traversable route.
- `terrain {x?,z?,radius?}` (controller-local): merged view of observed near-field surfaces and surveyed columns, `[x,z,y,kind,source,ageMs]`; absent columns are unknown.
- `survey {radius?,cursor?}` → `{origin,yawDegrees,radius,more,cursor,incomplete,columns}`: one sight-verified surface sample per column, 360° within 8 blocks plus the forward 120° cone to radius (8–64, default 48). Rows `[x,z,y,kind,step,code]`; `y` is the standing surface top, `kind` ground|canopy|water|hazard, `step` the absolute-aligned sampling spacing (1 to 16 blocks, 2 to 32, 4 beyond). Absent columns are unknown, never air; nothing below the visible surface. Same paging, budget and expiry rules as `scan`; two cursors. Node keeps columns in `SurfaceMemory` (5 min) and `travel` plans coarse corridors over them (`success|partial|noPath`), handing bounded legs to the fine navigator, which still validates every step against observed nearby terrain.
