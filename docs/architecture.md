...
- `sense {session?,after?}` → `{state,terrain:{session,reset,cursor,more,clock,cells}}`. Max 128 cells/page. Rows: `[x,y,z,observedTick,hazard,relativeCollisionBoxes|null]`; boxes `[x1,y1,z1,x2,y2,z2]`; null invalidates. `clock`/timestamps are mod monotonic ms, not UTC. Reset discards old memory; drain `more` before movement. Unchanged geometry is deduplicated and republishes freshness at most every 10 seconds; changes publish immediately. Air `[]` ≠ unknown. Nearby terrain sampling is camera-independent, range/occlusion bounded.
...
- `control_begin {owner,session,epoch,allowStarvingRecovery?}`: owner 32-hex UUID; observed life session/control epoch; exclusive 500ms initial lease. The optional recovery scope permits low health only while low food is also active; the lease retains that scope until release.
