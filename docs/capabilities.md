...
| Forage | `scan.objects[].forage` / `inspect_target.forage`: visible berry growth stage or normal mushroom state, ripe/actionable flag and food code; null = unsupported/unknown. `BEBehaviorFruitingBush.BState.Growthstate`; legacy berry/mushroom block variants. Safety allowlists remain in Node. Read only after LOS/native selection; no soil, traits or growth timers. |
