...
| Nearby entities/items | GetEntitiesAround; EntityItem.Itemstack. Creature sightings retain the synchronized `Alive` state. `IProjectile` entities are reported as their carried item even when remote-client `ProjectileStack`/`Stuck` synchronization lags; motion is exposed and native `CanCollect` remains authoritative. Dropped items have IsInteractable=false; proximity pickup, not crosshair use. |
...
| Terrain | Block.GetCollisionBoxes(accessor, pos); Entity.CollisionBox for body clearance. Each cell also carries traits (`water`/`lava`/`fire` from the fluid layer and block code, `leaves`, `plant` (BlockMaterial), `climbable`, `shape` for overflowing geometry, `tierN` from GetRequiredMiningTier): facts Node reasons with, never behavior. IClientEventAPI.BlockChanged invalidates neighbor-dependent shapes; GetChunkAtBlockPos=null means unknown. |
...
| Block facts | sightings `extra.facts` / `target.facts`: `{name,variant,growth}` — `GetPlacedBlockName`, the code's variant states (`VariantStrict`), and `BEBehaviorFruitingBush.BState.Growthstate` where a fruiting plant shows its state by its model. Facts only, never purpose labels (no berry/mushroom/crop kinds, no ripe flag, no drop code); Node decides what a block is for from these and the handbook. Read only after LOS/native selection; no soil, traits or growth timers. |
| Handbook | `item_info {code}`: one page as the player reads it. `GetHeldItemName`, `GetNutritionProperties` (satiety, health, category, intoxication, psychedelic), `Tool`/`ToolTier`/`GetMaxDurability`, `attributes.backpack.quantitySlots`, `CombustibleProps`, `GetDropsForHandbook`, `BlockBehaviorHarvestable`/`BlockBehaviorFruitingBush.harvestedStacks` (with the growth state a harvest needs), and `CollectibleBehaviorHandbookTextAndExtraInfo.GetHandbookInfo` flattened to lines with item links as `[code]`; the game's typing as read: `class` (the C# class), `material` (`BlockMaterial`), `behaviors` (attached `BlockBehavior`/`CollectibleBehavior` class names), `miningTier`, `resistance`, `climbable`, `replaceable`, `liquid`. A creature code (`IWorldAccessor.GetEntityType`) gives `class`, native entity `tags` and `Drops`. Any code the handbook lists, like its search box; nothing about the world or the bot. Traits (what a thing affords) are derived in Node ([traits](../src/support/traits.ts)), never here. |
| Handbook search | `catalog {match?,type?,offset,limit,text?}`: the handbook's search box as data. Every loaded block/item via `IWorldAccessor.Collectibles` and every creature via `IWorldAccessor.EntityTypes` whose code or name contains `match`, stable-sorted by code, a page at a time; facts only unless `text:true` renders each page's first lines (slow, game thread). `search_items` reads it live; nothing is dumped or cached beyond the session. Feature flag `catalog_facts`. |
...
| World map | `WorldMapManager.MapLayers` → `WaypointMapLayer.ownWaypoints`: the player's own markers as the map screen lists them (death adds icon `gravestone`, title "You died here"). Removal sends the map dialog's own `/waypoint remove <index>` chat command; verify by re-reading. `ChunkMapLayer` writes exact 32×32 RGBA chunks to `Maps/<SavegameIdentifier>.db` as the full map or player-following minimap makes them visible; the operator keeps the minimap active and syncs that persistent cache without exposing unseen terrain. `SystemRemotePlayerTracking.GetAllTrackedPlayerPositions()` is the same server-filtered feed rendered by `PlayerMapLayer`; it honors `mapHideOtherPlayers`, `mapPlayerRenderDistance`, group visibility, and map permission. `map_view` reports `mode:closed|minimap|world`, the connection-scoped full-map `scanned` flag and a session that changes on reconnect. The operator may also capture the actual open game map from the verified window as a fallback; `WorldMapManager.TranslateWorldPosToViewPos` supplies screenshot calibration. |
...
- Containers: a world container is known only while the player has it open: `open_container` right-clicks the aimed block through the normal input path and reads the inventory the game opened (`OpenedInventories` + `HasOpened`), `move_container_item` moves through `TryTransferTo` with the container's own state token, `close_container` closes the dialog the way its close icon does. Never read a block entity's inventory without that open UI; `backpack` slots 0–3 are bag slots (`bag: true`).
