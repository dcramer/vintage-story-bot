...
export function terrainView(map, surface, center, radius, _now = Date.now()) {
  ...
  for (const cell of map.cells.values()) {
    ...
    const id = columnKey(cell.x, cell.z),
      prior = columns.get(id);
    if (cell.hazard) {
      if (prior?.kind !== 'hazard' || prior.y < cell.y + 1)
        columns.set(id, { x: cell.x, z: cell.z, y: cell.y + 1, kind: 'hazard', source: 'observed', at: cell.at });
      continue;
    }
  ...
  const rows = [...columns.values()].sort(
    (a, b) => Math.hypot(a.x + 0.5 - center.x, a.z + 0.5 - center.z) - Math.hypot(b.x + 0.5 - center.x, b.z + 0.5 - center.z),
  );
  const counts = {};
  ...
  return {
    ...
    radius,
    known: rows.length,
    unknown: (2 * radius + 1) ** 2 - rows.length,
    counts,
    ...
    columns: rows.map(row => [
      ...
      row.kind,
      row.source,
      Math.max(0, (row.source === 'observed' ? map.now : surface.now) - row.at),
    ]),
  };
}
