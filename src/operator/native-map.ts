...
export function decodeChunkIndex(value) {
  ...
  let x = index & chunkMask,
    z = (index >> 27n) & chunkMask;
...
export function decodeMapPiece(input) {
  ...
  while (offset < bytes.length) {
    ...
    if (wire === 0) {
      ...
      offset = item.offset;
      values.push(Number(item.value & 0xffffffffn));
    } else if (wire === 2) {
...
export function readNativeMap(mapId, { dataRoot = path.join(root, '.runtime/bot-data') } = {}) {
  ...
  try {
    return db
      .prepare('SELECT position,data FROM mappiece ORDER BY position')
      .all()
      .map(row => {
        const pixels = decodeMapPiece(row.data),
          { x, z } = decodeChunkIndex(row.position);
        return {
          x,
          z,
          pixels,
          hash: createHash('sha256')
            .update(row.data as Buffer)
            .digest('base64url')
            .slice(0, 16),
        };
      });
...
async function upload(body) {
  const url = process.env.VINTAGE_STORY_REPORT_URL,
  ...
  const response = await fetch(new URL('/api/native-map', url), {
    ...
    body: JSON.stringify({ id: process.env.VINTAGE_STORY_BOT_ID, ...body }),
...
export async function syncNativeMap({ known = new Map(), sample = null } = {}) {
  ...
  const world = sample.map.id,
    ...
    changed = chunks.filter(chunk => known.get(`${world}:${chunk.x}:${chunk.z}`) !== chunk.hash);
  ...
  for (let index = 0; index < batches.length; index++) {
    ...
    result = await upload({
      world,
      at: Date.now(),
      players: index === batches.length - 1 ? players : undefined,
      complete: index === batches.length - 1,
      chunks: batch.map(chunk => [chunk.x, chunk.z, chunk.pixels.toString('base64')]),
    });
