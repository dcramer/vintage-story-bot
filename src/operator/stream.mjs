...
import { currentDisplay, listProcesses, root, tool } from './display.mjs';
...
function killStale(options) {
  ...
  for (const { pid, argv } of listProcesses()) {
    ...
    if (stale) { try { process.kill(pid, 'SIGTERM'); } catch {} }
  }
}

// Starts the supervisor; `onChange(status)` fires whenever server or encoder liveness changes.
export function superviseStream({ fps = defaults.fps, kbps = defaults.kbps, bind = defaults.bind, onChange = () => {}, log = () => {} } = {}) {
  const options = { ...defaults, fps, kbps, bind };
  const ffmpeg = tool('ffmpeg');
  const missing = !ffmpeg ? 'ffmpeg is missing; install it with your package manager.'
    : !existsSync(paths.mediamtx) ? `${paths.mediamtx} is missing; run scripts/setup-linux.sh.` : null;
  let server = null, encoder = null, display = null, codec = null, stopping = false, reason = missing;
  const status = () => ({
    streaming: Boolean(server && encoder), server: Boolean(server), encoder: Boolean(encoder), display, codec, reason,
    fps, kbps, hosts: hosts(),
    urls: { rtsp: `rtsp://HOST:${options.rtspPort}/${options.name}`, hls: `http://HOST:${options.hlsPort}/${options.name}/`, webrtc: `http://HOST:${options.webrtcPort}/${options.name}/` },
  });
  const changed = () => onChange(status());

  async function ensureServer() {
    if (server) return true;
    if (await portOpen(options.rtspPort)) { killStale(options); await sleep(1000); }
    if (await portOpen(options.rtspPort)) { reason = `port ${options.rtspPort} is in use by another process`; return false; }
    ...
    child.once('exit', code => { if (server === child) { server = null; reason = `mediamtx exited (${code}); see ${paths.serverLog}`; log(reason); changed(); } });
    server = child;
    const deadline = Date.now() + 10_000;
    while (!(await portOpen(options.rtspPort))) {
      if (child.exitCode !== null || Date.now() > deadline) { if (child.exitCode === null) child.kill('SIGTERM'); return false; }
      await sleep(100);
    }
  ...
  async function ensureEncoder() {
    ...
    if (!screen) { reason = 'no display is serving; start the game'; return; }
    // The sign-in screen stays private: nothing typed there should reach an unauthenticated stream.
    if ((await gameStatus()).phase === 'login_required') { reason = 'client is on the sign-in screen'; return; }
    codec ??= chooseEncoder(ffmpeg);
    const out = openSync(paths.encoderLog, 'a');
    const child = spawn(ffmpeg, [
      '-hide_banner', '-loglevel', 'warning', '-nostdin',
      '-f', 'x11grab', '-framerate', String(fps), '-video_size', `${screen.width}x${screen.height}`, '-draw_mouse', '0', '-i', screen.display,
      ...encoderArgs(codec, fps, kbps),
      '-f', 'rtsp', '-rtsp_transport', 'tcp', `rtsp://127.0.0.1:${options.rtspPort}/${options.name}`,
    ], { cwd: dir, stdio: ['ignore', out, out], env: { ...process.env, DISPLAY: screen.display } });
    child.once('exit', code => { if (encoder === child) { encoder = null; reason = `ffmpeg exited (${code}); see ${paths.encoderLog}`; log(reason); changed(); } });
    encoder = child; display = screen.display; reason = null;
  ...
  async function tick() {
    ...
    try { if (await ensureServer()) await ensureEncoder(); }
    catch (error) { reason = error.message; log(reason); }
  }
