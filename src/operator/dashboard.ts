...
import { bridgePort } from '../runtime/bridge.ts';
import { superviseNativeMap } from './native-map.ts';
import { superviseStream } from './stream.ts';
...
const snapshot = () => ({
  ...
  topics: Object.fromEntries(latest),
  log,
  stream: stream?.status() ?? null,
  nativeMap: nativeMap?.status() ?? null,
});
...
const server = http.createServer((request, response) => {
  ...
  if (request.method === 'POST' && pathname === '/ingest') {
    ...
    broadcast('producers', { count: producers.size });
    let buffer = '';
    ...
    request.on('data', chunk => {
      ...
      while ((end = buffer.indexOf('\n')) >= 0) {
        ingest(buffer.slice(0, end).trim());
        buffer = buffer.slice(end + 1);
      }
      if (buffer.length > maxLine) request.destroy();
    });
    request.on('end', () => {
      if (buffer.trim()) ingest(buffer.trim());
      json(response, 200, { ok: true });
    });
    request.on('close', () => {
      if (producers.delete(request)) broadcast('producers', { count: producers.size });
    });
    return;
  }
...
stream = superviseStream({
  bind: process.env.VINTAGE_STORY_STREAM_BIND || undefined,
  onChange: status => broadcast('stream', status),
  log: line => console.error(`[stream] ${line}`),
});
