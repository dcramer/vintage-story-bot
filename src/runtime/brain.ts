...
export type Reading = {
  ...
  active: { id: string; kind: string; state: string; by: string; args?: Record<string, unknown>; progress?: Record<string, unknown> } | null;
...
export class BrainLoop<Memory> {
  ...
  private async tick() {
    ...
    const active = record
      ? { id: record.id, kind: record.kind, state: record.state, by: record.by, args: record.args, progress: record.progress }
      : null;
