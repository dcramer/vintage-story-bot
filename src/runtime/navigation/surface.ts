...
export function edgeCost(surface, from, to, { canopyCost = 1, shoreCost = 2, slopeCost = 1.5 } = {}) {
  ...
  return d + Math.abs(rise) * slopeCost + (to.kind === 'canopy' ? d * canopyCost : 0) + (surface.shore(to) ? shoreCost : 0);
}
...
export function planRoughRoute(surface, start, goal, { budget = 4096, penalty = (_column: any): number => 0, minimumProgress = 4 } = {}) {
  ...
  const remaining = column => Math.hypot(column.x + 0.5 - goal.x, column.z + 0.5 - goal.z);
...
export function simplify(columns, maxSpan = 12, turnDegrees = 20) {
  const points = columns.map(c => ({ x: c.x + 0.5, y: c.y, z: c.z + 0.5, kind: c.kind, step: c.step }));
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    if (i === points.length - 1 || turn > turnDegrees || span >= maxSpan) {
      ...
      if (i === points.length - 1 || span >= maxSpan) {
        result.push(p);
        span = 0;
