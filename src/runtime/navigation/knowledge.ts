...
export class Knowledge {
  dir: any; memories: any;
  identifier = null; dirty = false; savedAt = 0; loaded = null;
  constructor(dir, memories) { this.dir = dir; this.memories = memories; }
  file(identifier) { return join(this.dir, `${encodeURIComponent(identifier)}.json`); }
  ...
  save(force = false, now = Date.now()) {
    ...
    const { map, surface, sightings } = this.memories;
    const data = { version: 1, identifier: this.identifier, savedAt: now,
      terrain: map.export(), surface: surface.export(), sightings: sightings.export() };
    mkdirSync(this.dir, { recursive: true });
    const target = this.file(this.identifier), temporary = `${target}.${process.pid}.tmp`;
    writeFileSync(temporary, JSON.stringify(data));
    renameSync(temporary, target);
    this.dirty = false; this.savedAt = now;
    return true;
  }
  status() {
    return { identifier: this.identifier, dirty: this.dirty, savedAt: this.savedAt, loaded: this.loaded,
      cells: this.memories.map.cells.size, columns: this.memories.surface.columns.size, sightings: this.memories.sightings.records.size };
  }
}
