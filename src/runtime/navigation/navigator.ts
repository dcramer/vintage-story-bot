...
import { angle, horizontal, key, lookAt, normalize, JUMP_HEIGHT, MAX_DROP, STEP_HEIGHT } from './terrain.ts';
...
export class Navigation {
  bestNear: any; deadline: any; desiredYaw: any; diagnostics: any; edgeStart: any; eyeHeight: any; height: any; lastReplan: any; map: any; nextCheckpoint: any; primaryTarget: any; progressAt: any; replannedAt: any; steppedOff: any; surveyAt: any; target: any; width: any; yawError: any;
  id = randomUUID(); state = 'surveying'; reason = null; events: any[] = [];
  route = []; index = 0; replans = 0; segments = 0;
  blocked = new Set(); visits = new Map();
  lookingAt = null;
  jumpAt = 0; airborne = false; nextPlanAt = 0;
  ...
  tick(state, now = Date.now()) {
    ...
    if (grounded && horizontal(p, this.target) < (this.target.arrivalRadius ?? .3) &&
        (this.target.horizontalOnly || Math.abs(p.y - this.target.y) < .6)) {
    ...
    if (this.state === 'surveying') {
      ...
      if (!planned) {
        if (now - this.surveyAt < 2500) {
          this.lookingAt = [...map.views(p).values()]
    ...
    const reached = node => {
      ...
      return along >= 0 && along < 2.5 && lateral < .5 && Math.abs(p.y - node.y) < .6;
    };
    ...
    if (grounded && !this.jumpAt) for (let ahead = this.index + 1; ahead < this.route.length; ahead++) {
    ...
    const still = map.nodeAt(Math.floor(next.x), Math.floor(next.z), next.y, .1, .1);
    if (!still) {
    ...
    const sprint = !!this.target.sprint && grounded && !this.jumpAt && straight &&
      food?.max > 0 && food.current / food.max >= (emergency ? .1 : .6);
    return { yawDegrees, pitchDegrees: 15, forward: walking && near > .12, durationMs,
      jump: !!this.jumpAt && now - this.jumpAt < 200, sprint, sneak: false };
  }
}
