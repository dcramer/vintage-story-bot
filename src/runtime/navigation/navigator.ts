...
// A run of checkpoints is merged into one up to this far ahead. The bridge validates
// against the body's live position, while this process only has the position returned
// by the preceding frame. On a remote server a sprinting body can travel roughly two
// blocks before the next request is handled, so leave three blocks inside its eight-
// block control boundary.
export const MERGE_RUN = 5;
const QUEUED_POINT_RANGE = 5;
...
export class Navigation {
  ...
  tick(state, now = Date.now(), step: any = null) {
    ...
    let next2 =
      rollOn && distance(p, after) <= QUEUED_POINT_RANGE
