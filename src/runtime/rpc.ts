import { bridgePort, requestBridge } from './bridge.ts';
...
export async function requestController(request) {
  try {
    return await requestBridge(request, { port: controllerPort(), timeoutMs: 10000, requestMaxBytes: 16384 });
  } catch (error) {
    throw new Error(`Controller: ${error.message} Start pnpm controller; never retry a mutation blindly.`);
  }
}

export async function runGoal(request: object, { signal }: { signal?: AbortSignal } = {}) {
  ...
  while (true) {
    if (signal?.aborted) {
      await requestController({ action: 'stop', expectedGoal: id });
