...
export class Telemetry {
  pending = []; latest = new Map(); request = null; ready = false; timer = null; closed = false;
  port: number; flushMs: number; retryMs: number; limit: number;
  ...
  publish(topic, data, { coalesce = false } = {}) {
    ...
    else { this.latest.delete(topic); this.pending.push(line); if (this.pending.length > this.limit) this.pending.shift(); }
    this.timer ??= setTimeout(() => { this.timer = null; this.flush(); }, this.flushMs).unref();
  }
  flush() {
    if (!this.ready || !this.request?.writable) return;
    const lines = [...this.pending, ...this.latest.values()];
    this.pending.length = 0; this.latest.clear();
    if (lines.length) this.request.write(lines.join('\n') + '\n');
  }
  connect() {
    if (this.closed) return;
    const request = http.request({ host: '127.0.0.1', port: this.port, method: 'POST', path: '/ingest',
      headers: { 'content-type': 'application/x-ndjson' }, agent: false });
    const retry = () => {
      if (this.request !== request) return;
      this.request = null; this.ready = false; request.destroy();
      setTimeout(() => this.connect(), this.retryMs).unref();
    };
    request.on('error', retry); request.on('close', retry);
    request.on('response', response => response.resume());
    request.once('socket', socket => { socket.unref(); socket.once('connect', () => { if (this.request === request) { this.ready = true; this.flush(); } }); });
  ...
  close() {
    ...
    if (this.ready) request?.end(); else request?.destroy();
  }
}
