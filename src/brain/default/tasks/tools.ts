...
export function makeTool(k: Kit, tool: string, head: string, blades: number, output: string): Decision {
  if (blades < 1) {
    ...
    if (!k.stone)
      return {
        start: 'gather',
        args: { match: 'looseflints', item: 'game:flint', count: Math.max(1, missing(k)), timeoutMs: 600000 },
        why: `flint for the ${tool}`,
      };
...
const toolTask = (id: 'knife' | 'axe' | 'shovel', head: string, blades: (k: Kit) => number, output: string): Concern => ({
  ...
  wants: k => (!k[id] && !k.stone ? KNAPPABLE_WANTS : []),
  short: k => (!k[id] && !k.stone ? { item: 'game:flint', count: missing(k) } : null),
});
