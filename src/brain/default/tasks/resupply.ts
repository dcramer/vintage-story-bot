// What the chest was last seen holding that the kit is short of is fetched
...
export function resupplyOf(k: Kit, s: { home: boolean; torches: number }, stash: Stash | null): { item: string; count: number }[] {
  ...
  const held = (item: string) =>
    Object.entries(stash.seen!.items)
      ...
      .reduce((n, [, q]) => n + q, 0);
  // One want per item: the three tool tasks all name flint, and the largest shortfall is the whole of it.
  const wants = new Map<string, number>();
  for (const want of FEEDS.map(task => task.short?.(k, s)))
    if (want && want.count > 0) wants.set(want.item, Math.max(wants.get(want.item) ?? 0, want.count));
  return [...wants].map(([item, count]) => ({ item, count: Math.min(count, held(item)) })).filter(want => want.count > 0);
}
