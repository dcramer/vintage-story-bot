import { horizontal, lookAt, angle, key } from '../runtime/navigation/terrain.mjs';
import { changeBlock, selectCell } from './blocks.mjs';
...
export function reachable(map, origin, limit = pitLimit) {
  const seen = new Set([key(origin)]), queue = [origin];
  while (queue.length && seen.size < limit) {
    const at = queue.shift();
    for (const { node } of [...map.moves(at), ...map.gapMoves(at)]) {
      const id = key(node);
      if (seen.has(id)) continue;
      seen.add(id); queue.push(node);
    }
  }
  return seen.size;
}

const solid = (map, x, y, z) => { const c = map.get(x, y, z); return !!c && !c.hazard && c.boxes.some(b => b[4] - b[1] > .99 && b[3] - b[0] > .99 && b[5] - b[2] > .99); };
...
export function stairStep(map, node, toward) {
  const x = Math.floor(node.x), z = Math.floor(node.z), h = Math.floor(node.y);
  const heading = lookAt(node, toward).yawDegrees;
  const directions = cardinals.map(([dx, dz]) => ({ dx, dz, off: Math.abs(angle(lookAt(node, { x: node.x + dx, z: node.z + dz }).yawDegrees, heading)) }))
...
export async function digOut(field, toward, { steps = 8 } = {}) {
  ...
  for (let step = 0; step < steps; step++) {
    ...
    for (const cell of plan.dig) {
      ...
      if (!selected) { cut = false; break; }
      const slot = await diggingSlot(field, selected, inventory);
      if (slot === null) { field.report('cannot_dig', { cell, code: selected.code, tier: selected.requiredMiningTier }); cut = false; break; }
