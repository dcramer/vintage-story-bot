...
export async function changeBlock(field, kind, { target, point, face, slot, expectedItem, acceptTransform = false, timeoutMs = 60000 }: BlockChange) {
  ...
  while (true) {
    await field.observe();
    if (operation.state === 'cancelled' || operation.state === 'failed') return { ok: false, reason: operation.reason, operation };
    ...
    operation = await field.send({
      action: kind === 'dig' && operation.state === 'working' ? 'block_action_continue' : 'block_action_status',
      id,
      sequence: ++sequence,
    });
  }
