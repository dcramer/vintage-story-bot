...
export const leafBlock = object => object?.kind === 'block' &&
  /^game:leaves(?:branchy)?-/.test(object.code ?? '');
export const threatAllowsLeafClearing = (state, threat, minimum = 12) => !threat ||
...
export function leafClearCandidate(objects, state, toward, skipped = new Set()) {
  ...
  const candidates = objects.filter(object => !skipped.has(object.key) && leafBlock(object) && object.withinPickingRange &&
...
export async function clearLeaf(field, toward) {
  ...
  } catch { /* A changed/occluded leaf is simply not a verified clearing. */ }
