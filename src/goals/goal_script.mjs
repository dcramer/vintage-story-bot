...
export default defineGoal({
  ...
  schema: z.object({
    ...
    goalScript: z.string().min(1).max(8192).describe(
      'TypeScript-compatible sequence of `await goals.<goal>({ ...literalArgs });` calls. Use api for goal schemas.',
    ),
  }).strict().superRefine(({ goalScript }, context) => {
    try { parseGoalScript(goalScript); }
    catch (error) { context.addIssue({ code: 'custom', path: ['goalScript'], message: error.message }); }
  }),
  destructive: true,
  description:
