...
import { box } from '../support/structures.ts';
...
export default defineGoal({
  ...
  schema: z
    ...
    .strict()
    .refine(a => (a.cells !== undefined) !== (a.box !== undefined), 'Supply exactly one of cells or box')
    .refine(
      a =>
        !a.box ||
        (Math.abs(a.box.to.x - a.box.from.x) + 1) * (Math.abs(a.box.to.y - a.box.from.y) + 1) * (Math.abs(a.box.to.z - a.box.from.z) + 1) <= 64,
      'box exceeds 64 cells',
    ),
  destructive: true,
  description:
  ...
  run: (env, { cells, box: bounds, ...options }) =>
    runField(env, options, ['inventory', 'block_actions'], (field, survival, o) =>
      digArea(field, survival, { ...o, cells: cells ?? box(bounds.from, bounds.to) }),
    ),
});
