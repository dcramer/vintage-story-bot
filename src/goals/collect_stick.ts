...
export async function collectStick(send, wait = ms => new Promise(resolve => setTimeout(resolve, ms))) {
  ...
  if (
    !before.ok ||
    !before.capabilities?.includes('target_guard') ||
    !before.capabilities?.includes('life_events') ||
    !before.alive ||
    before.paused ||
    !(before.controlReady ?? before.mouseGrabbed) ||
    before.life?.alerts?.length
  ) {
  ...
  const safe = state =>
    ...
    state.alive &&
    !state.paused &&
    (state.controlReady ?? state.mouseGrabbed) &&
    state.player.uid === before.player.uid &&
    state.life?.session === before.life?.session &&
    state.life?.lastDamageAt === before.life?.lastDamageAt &&
    !state.life?.alerts?.length;
  const count = state =>
    [...state.hotbar, ...state.backpack].filter(slot => slot.code === 'game:stick').reduce((total, slot) => total + slot.quantity, 0);
  ...
  try {
    ...
    if (!safe(after) || after.target?.key === target.key || count(after) <= count(before)) {
