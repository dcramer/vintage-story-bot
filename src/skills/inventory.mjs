...
export const itemCount = (inventory, code) => ownedSlots(inventory)
  .filter(s => s.code === code).reduce((n, s) => n + s.quantity, 0);
export const carriedCount = (state, code) => [...state.hotbar.filter(s => s.slot < 10), ...state.backpack]
  .filter(s => s.code === code).reduce((n, s) => n + s.quantity, 0);

export async function equip(field, { item, tool, minTier = 0, slot }) {
  await field.observe();
  let inventory = await field.send({ action: 'inventory' });
  const slots = ownedSlots(inventory);
  const matches = s => tool !== undefined
    ? s.code && s.tool === tool && s.toolTier >= minTier && s.durability > 0
    : s.code === item;
  const source = slots.filter(s => matches(s) && (s.code ? s.quantity > 0 : s.inventory === 'hotbar'))
    .sort((a, b) => (tool ? a.toolTier - b.toolTier : 0) ||
      Number(b.inventory === 'hotbar') - Number(a.inventory === 'hotbar') ||
      Number(b.slot === slot) - Number(a.slot === slot) || (b.durability ?? 0) - (a.durability ?? 0))[0];
  if (!source) throw Error('No matching owned item/tool or empty hand slot');
  let destination = slot === undefined
    ? source.inventory === 'hotbar' ? source : slots.find(s => s.inventory === 'hotbar' && !s.code)
  ...
  if (!same && source.code) {
    ...
    await field.observe();
    await field.send({ action: 'inventory_move', from, to: { inventory: 'hotbar', slot: destination.slot },
      quantity: 1, expectedState: inventory.state });
    let verified = false;
    for (let i = 0; i < 10; i++) {
      ...
      if (received?.code === source.code && received.quantity === 1 && matches(received) &&
        ...
        inventory = contents; destination = received; moved = 1; verified = true; break;
      }
    }
  ...
  return { ok: true, goal: 'equip', slot: held.slot, item: held.code, tool: held.tool,
    toolTier: held.toolTier, durability: held.durability, moved, from, verification: 'client_observed' };
}
